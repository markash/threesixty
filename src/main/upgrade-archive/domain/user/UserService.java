...
//		this.identityService = identityService;
	...
	public void init() {
		...
		Optional<Role> role = roleRepository.findById(Role.ROLE_ADMIN);
		Role adminRole = role.orElseGet(() -> roleRepository.save(new Role(Role.ROLE_ADMIN)));
		
		role = roleRepository.findById(Role.ROLE_USER);
		Role userRole = role.orElseGet(() -> roleRepository.save(new Role(Role.ROLE_ADMIN)));
		
		/* Ensure that the administrator user is configured */
		Optional<User> user = userRepository.findById(User.USER_ADMIN);
		User administrator;
		if (user.isEmpty()) {
			administrator = new User(User.USER_ADMIN, User.USER_ADMIN_PASSWORD, adminRole)
					.withFirstName("System")
					.withLastName("Administrator")
					.withEmail("admin@localhost")
					.withCreatedDate()
					.withPasswordChangeRequired(false);
		} else {
		...
		for(Position position : samplePositions) {
			Optional<Position> persisted = positionRepository.findOne(Example.of(position));
			if (persisted.isEmpty()) {
				Optional<JobProfile> profile = jobProfileRepository.findOne(Example.of(position.getJobProfile()));
				if (profile.isEmpty()) {
...
//				"Server started.");
	...
	public Response<User> authenticate(final String userName, final String password) {
		Optional<User> user = userRepository.findOne(Example.of(User.EMPTY().withId(userName)));
		if (user.isEmpty()) { return new Response<>(RequestResult.UNAUTHORIZED.setDescription("The user is invalid")); }
		if (!user.get().getPassword().equals(password)) { return new Response<>(RequestResult.UNAUTHORIZED.setDescription("The user is invalid")); }
		
		user.get().retrievePictureSilently(client);
		return new Response<>(RequestResult.OK, user.get());
	}
	
	public Response<User> changePassword(final String userName, final String oldPassword, final String newPassword) {
		Optional<User> persisted = userRepository.findOne(Example.of(User.EMPTY().withId(userName)));
		if (persisted.isEmpty()) { return new Response<>(RequestResult.UNAUTHORIZED.setDescription("The user is invalid")); }
		if (!persisted.get().getPassword().equals(oldPassword)) { return new Response<>(RequestResult.UNAUTHORIZED.setDescription("The user is invalid")); }
	...
	public Response<User> resetPassword(
			final String userId,
			final User changedBy) {

		Optional<User> persisted = findUser(userId);
		if (persisted.isEmpty()) {
			return new Response<>(RequestResult.INVALID_USER_ID.setDescription("The user " + userId +  " is invalid"));
		}
		User user = persisted.get();
		user.setPasswordChangeRequired(true);
		user.setLastModifiedBy(changedBy.getId());
		user.setLastModifiedDate(LocalDateTime.now());
		return new Response<>(RequestResult.OK, userRepository.save(user));
	}
	
	public Optional<User> findUser(final String id) {
		Optional<User> user = userRepository.findOne(Example.of(User.EMPTY().withId(id)));
		if (user.isPresent()) {
			try {
	...
	public List<User> findUsers() {
		...
		return userRepository.findAll(Example.of(example), Sort.by(Sort.Order.asc("id")));
	}
	...
	public List<Country> findCountries() {
		return countryRepository.findAll(Sort.by("commonName"));
	}
...
//		User example = new User();
//		example.setDepartment(identity);
//		return this.userRepository.findAll(Example.of(example), Sort.by(Sort.Order.asc("id")));
//	}
	...
	public int getUnreadNotificationsCount(@NotNull final User user) {
		...
		return this.userNotificationRepository
				.findAll(Example.of(new UserNotification(user)))
				.size();
	}
	...
	public List<UserNotification> findNotifications(@NotNull final User user, int limit) {
		...
		List<UserNotification> notifications =
				this.userNotificationRepository
						.findAll(Example.of(new UserNotification(user)), Sort.by(Sort.Order.desc("time")));
